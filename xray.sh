...
install_shortcuts() {
    ...
    if ! grep -q "alias xray=" /root/.bashrc 2>/dev/null; then
        echo "alias xray='/usr/local/bin/xray-panel'" >> /root/.bashrc
    fi
    echo "alias xray='/usr/local/bin/xray-panel'" > /etc/profile.d/xray.sh 2>/dev/null
    chmod +x /etc/profile.d/xray.sh 2>/dev/null
}
...
header() {
    ...
    echo -e "${CYAN}${BOLD}════════════════════════════════════════════════════════════${NC}"
    echo -e " ${PURPLE}${BOLD}▸ IP Servidor:${NC}  ${YELLOW}${BOLD}${SERVER_IP}${NC}"
...
show_info() {
    if [[ ! -f "$CONFIG_FILE" ]]; then
        echo -e "${RED}${BOLD}[✗] No hay ninguna configuración activa.${NC}"
        pause_screen
        return
    fi
...
configure_protocol() {
    header
    echo -e "${PURPLE}${BOLD}🌐 SELECCIONA UN PROTOCOLO PARA CONFIGURAR:${NC}\n"
    echo -e "  ${WHITE}${BOLD}[ 1 ]${NC} ${CYAN}${BOLD}VLESS${NC}"
    echo -e "  ${WHITE}${BOLD}[ 2 ]${NC} ${CYAN}${BOLD}VMess${NC}"
    ...
    header
    echo -e "${PURPLE}${BOLD}📝 DATOS DE CONEXIÓN (${proto^^}):${NC}\n"
    ...
    echo -e "  ${WHITE}${BOLD}[ 1 ]${NC} ${CYAN}TCP Directo${NC}"
...
PY
    ...
    if systemctl is-active --quiet xray; then
        echo -e "\n${GREEN}${BOLD}✔ Configuración aplicada y Xray reiniciado exitosamente.${NC}"
    else
        echo -e "\n${RED}${BOLD}❌ Error: Xray no pudo iniciar. Revisa los logs.${NC}"
...
while true; do
    ...
    case "$op" in
        ...
        2)
            ...
            echo -e "${GREEN}${BOLD}✔ Puerto actualizado a $np.${NC}"
            pause_screen
            ;;
        3)
        ...
        4)
            ...
            modify_param "id" "$nid"
            echo -e "${GREEN}${BOLD}✔ ID actualizada a $nid${NC}"
            pause_screen
            ;;
        5)
            ...
            echo -e "${GREEN}${BOLD}✔ ID $aid agregada correctamente.${NC}"
            pause_screen
            ;;
        6) show_info ;;
        7)
            echo -e "\n${YELLOW}${BOLD}Presiona CTRL + C para detener logs...${NC}\n"
            journalctl -u xray -f
            ;;
        8)
            systemctl restart xray >/dev/null 2>&1
            echo -e "${GREEN}${BOLD}✔ Xray reiniciado exitosamente.${NC}"
            sleep 1.5
            ;;
        9)
            echo -e "\n${YELLOW}${BOLD}⚡ Desinstalando Xray por completo...${NC}"
            systemctl stop xray 2>/dev/null
            systemctl disable xray 2>/dev/null
            rm -rf /usr/local/etc/xray /usr/local/bin/xray-panel /etc/profile.d/xray.sh
            sed -i '/alias xray=/d' /root/.bashrc 2>/dev/null
            systemctl daemon-reload
            rm -f "$LOCK_FILE"
            echo -e "${GREEN}${BOLD}✔ Desinstalación completa realizada.${NC}"
            exit 0
            ;;
        0) 
            rm -f "$LOCK_FILE"
