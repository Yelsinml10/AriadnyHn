...
main_menu() {
    while true; do
        ...
        printf "  %b[11]%b 🔒 %bSSL / TLS Manager%b\n\n" "$CYAN" "$NC" "$WHITE" "$NC"

        section_divider "GESTIÓN & MANTENIMIENTO"
        printf "  %b[12]%b 🛡️  %bFirewall%b           %b[13]%b 🔐 %bConfigurar SSH%b\n" "$CYAN" "$NC" "$WHITE" "$NC" "$CYAN" "$NC" "$WHITE" "$NC"
        printf "  %b[14]%b 📊 %bMonitoreo Sistema%b   %b[15]%b 📋 %bEstado General%b\n" "$BLUE" "$NC" "$WHITE" "$NC" "$BLUE" "$NC" "$WHITE" "$NC"
        printf "  %b[ 0]%b 🚪 %bSalir del Panel%b\n" "$RED" "$NC" "$WHITE" "$NC"

        read -r -p "  ❯ Selecciona una opción [0-15]: " option
