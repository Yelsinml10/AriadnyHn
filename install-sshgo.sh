...
build_http_list() {
    ...
    for i in "${ADDR[@]}"; do
        p=$(echo "$i" | tr -d ' ')
        # Se agrega http:// para forzar a Caddy a NO usar SSL en estos puertos
        [[ -n "$p" ]] && res="${res:+$res, }http://:${p}"
    done
    echo "$res"
}
...
build_http_list() {
    local ports="$1" res=""
    IFS=',' read -ra ADDR <<< "$ports"
    for i in "${ADDR[@]}"; do p=$(echo "$i" | tr -d ' '); [[ -n "$p" ]] && res="${res:+$res, }http://:${p}"; done
    echo "$res"
}
    ...
    case $op in
        ...
        4)
            ...
            if [ -n "$new_https" ]; then
                ...
                generate_caddyfile "$DOMAIN" "$HTTPS_PORTS" "$HTTPS_PORTS"
                systemctl restart caddy
                echo -e "${GREEN}✔ Actualizado.${NC}"
            fi
            read -r _ ;;
